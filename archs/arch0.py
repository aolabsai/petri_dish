...
input_channel_size = ((2*RADIUS + 1)**2) #- 1 # of grid points around agent counted as input
OUTPUT_SIZE = 100
...
def updateArch(stimuli_intensity):#, pain_threshold, pleasure_threshold):
    ...
    arch = ao.Arch(
        arch_i=([input_channels]*neurons_per_input_channel) +[2]+[2], # 27 total input neurons
        arch_z=[1]*OUTPUT_SIZE, # 1 output neuron, corresponding to: 1="go forward", 0="turn"
#        arch_c=[2], # 2 custom control neurons, as instinct neuron for pleasure-from-food and pain-from-hunger
        connector_function="forward_full_conn", # connection of neurons to each other
